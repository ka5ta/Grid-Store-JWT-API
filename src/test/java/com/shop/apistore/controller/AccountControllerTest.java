...
class AccountControllerTest {
    ...
    Account INPUT_1 = new Account("user@email.com", "12345", "USER");
    Account INPUT_2 = new Account("admin@email.com", "password", "ADMIN");
    Account INPUT_3 = new Account("JaneDoe@gmail.com", "pass", "USER");

    Account RECORD_1 = new Account(1L, INPUT_1.getEmail(), passwordEncoder.encode(INPUT_1.getPassword()),
            INPUT_1.getRole());
    Account RECORD_2 = new Account(2L, INPUT_2.getEmail(), passwordEncoder.encode(INPUT_2.getPassword()),
            INPUT_2.getRole());
    Account RECORD_3 = new Account(3L, INPUT_3.getEmail(), passwordEncoder.encode(INPUT_3.getPassword()),
            INPUT_3.getRole());
    ...
    class getAllAccountsTest {
        ...
        void getAllAccountsTest() throws Exception {
            ...
            MvcResult result = mockMvc
                    .perform(MockMvcRequestBuilders.get(ADMIN_GET_ALL_ACCOUNTS).header(HttpHeaders.AUTHORIZATION,
                            ADMIN_TOKEN))
                    .andDo(print()).andExpect(status().isOk())
                    .andExpect(jsonPath("$[0].email", is(RECORD_1.getEmail())))
                    .andExpect(jsonPath("$[1].email", is("admin@email.com")))
                    .andExpect(jsonPath("$[2].email", is("JaneDoe@gmail.com"))).andExpect(jsonPath("$", hasSize(3)))
        ...
        void getAllAccountsByUserFailTest() throws Exception {
            ...
            mockMvc.perform(
                    MockMvcRequestBuilders.get(ADMIN_GET_ALL_ACCOUNTS).header(HttpHeaders.AUTHORIZATION, USER_TOKEN))
                    .andDo(print())
                    .andExpect(jsonPath("$.error",
                            org.hamcrest.Matchers.is("Access Denied. You are not authorized to see content.")))
                    .andExpect(jsonPath("$.timestamp").value(notNullValue())).andExpect(status().isForbidden())
        ...
        void getAllAccountsAnonymousFailTest() throws Exception {
            when(accountServiceMock.getAllAccounts()).thenReturn(Arrays.asList(RECORD_1, RECORD_2, RECORD_3));
            mockMvc.perform(MockMvcRequestBuilders.get(ADMIN_GET_ALL_ACCOUNTS)).andDo(print())
    ...
    class changePasswordTest {
        ...
        void changePasswordUserSuccessTest() throws Exception {
            // Mock
            doNothing().when(accountServiceMock).changePassword(USER_CHANGE_PASS_INPUT, principalMock);

            mockMvc.perform(
                    MockMvcRequestBuilders.post(AUTHORIZE_CHANE_PASSWORD).contentType(MediaType.APPLICATION_JSON)
                            .header(HttpHeaders.AUTHORIZATION, USER_TOKEN).content(ChangePasswordDTO_USER))
        ...
        void changePasswordAdminSuccessTest() throws Exception {
            ...
            mockMvc.perform(
                    MockMvcRequestBuilders.post(AUTHORIZE_CHANE_PASSWORD).contentType(MediaType.APPLICATION_JSON)
                            .header(HttpHeaders.AUTHORIZATION, ADMIN_TOKEN).content(ChangePasswordDTO_USER))
                    .andDo(print()).andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.message", org.hamcrest.Matchers.is("Password was changed.")));
        ...
        void changePasswordAnonymousFailureTest() throws Exception {
            ...
            mockMvc.perform(MockMvcRequestBuilders.post(AUTHORIZE_CHANE_PASSWORD)
                    .contentType(MediaType.APPLICATION_JSON).content(ChangePasswordDTO_USER)).andDo(print())
